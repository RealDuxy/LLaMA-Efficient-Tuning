...
system = """你是一个具有丰富保险专业知识和常识的专家，你的任务是从不同角度将用户提出的问题改写为不同的形式，帮助用户克服基于语义相似度的检索方法的限制，并获取更多的信息。你的输出需要以Python List格式输出。"""
...
def preprocess_excel_data_and_convert_to_dataset(file_name, dev_ratio=0):
    train_datas = []
    dev_datas = []
    ...
    for i, data in tqdm(enumerate(data_list)):
        ...
        output = data["rewrite_output"].strip()
        if not output:
            continue
...
if __name__ == '__main__':
    ...
    train_datas, dev_datas = preprocess_excel_data_and_convert_to_dataset("0312_1000_input_gen.xlsx", 0)
    print("done")
    print(f"train datasets: {len(train_datas)}")
    print(f"eval datasets: {len(dev_datas)}")
    json.dump(train_datas, open("rephrase_query_askbob_gpt4_1k_0312.json", "w"), ensure_ascii=False, indent=4)
