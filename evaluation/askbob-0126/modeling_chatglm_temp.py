...
class ChatGLM3MultiScalingRotaryEmbedding(nn.Module):
    def __init__(self, dim, scaling_type, scaling_factor, max_seq_length, original_impl=False, device=None, dtype=None):
        super().__init__()
        inv_freq = 1.0 / (10000 ** (torch.arange(0, dim, 2, device=device).to(dtype=dtype) / dim))
        self.register_buffer("inv_freq", inv_freq)
        self.dim = dim
    ...
    def forward_impl(
        ...
        if self.original_impl:
            ...
            seq_idx = torch.arange(seq_len, dtype=torch.float, device=device)
            seq_idx / self.scaling_factor
        elif self.scaling_type == "NTK-by-Part":
            ...
