...
class BaseLiteLLMAgent:
    ...
    def assemble_messages(self, **kwargs):
        ...
        history = chat_templates["history"]
        prompt_content = chat_templates["prompt"]

        for key, value in kwargs.items():
            temp_value = value
            temp_key = "{" + key + "}"
            if temp_key not in prompt_content:
                if key not in gen_kwargs_list:
                    print(f"input_key: {temp_key} not in prompt, check you keys and template")
                continue
            prompt_content = prompt_content.replace(temp_key, temp_value)
        return history, prompt_content
    ...
    def para_invoke(self, **kwargs):
        ...
        try:
            input_kwargs = [
                {key: values[i] for key, values in kwargs.items()} for i in range(args_length)
            ]
        except IndexError:
